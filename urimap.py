import os, sys, json, time, datetime, spotipy, requests
...
try:
# tries to open the secret config json file
    with open(secretConfigPath, "r", encoding="utf-8") as scCfg:
    # opens the secret config file in read mode
        secretConfig = json.load(scCfg)
        # stores the loaded json file as secretConfig
except Exception as err:
# if there's an error
    print(f"Error reading the secret config file: {err}")
    # user inform
...
if os.path.exists(uriPath):
    ...
    with open(uriPath, "r", encoding="utf-8") as urimap:
...
def mapper(start, end):
    ...
    # gets the URI (track ID) for every unique entry in the CSV
        ...
        # for every track URI not found in the pre-existing URI mapped JSON file
            ...
            # if it fails, saves the error
                ...
                # if it's not 429
                    None
                    # does nothing (can't do much anyway)
            
            except PermissionError as error:
            # any firewall/network issue (unfortunately common, while testing at least)
                ...
                # if the error is specifically a windows 10013 error (connection blocked for one reason or another)
                    ...
                    print(f"{Time()} [INFO]: Sometimes this is due to odd Windows behavior, and a computer restart may fix this")
                    print(f"{Time()} [EXIT]: Exiting app automatically... (feel free to close)")
                    # spams a few user informs
                    time.sleep(600)
                    # sleeps for 600 seconds (10 minutes) to ensure user sees prompt
                    raise SystemExit
                else:
    ...
    with open(uriPath, "w", encoding="utf-8") as newUri:
...
print(f"{Time()} [EXIT]: Task complete - exiting app... (feel free to close)")
# user update
time.sleep(600)
# waits so it doesn't look like crash